...
```bash
npm test
node scripts/build-release.mjs 0.2.1 b3
git tag v0.2.1
...
- `agentlimb-chrome-v0.2.1-b3.zip`
- `SHA256SUMS.txt`

The extension manifest remains the numeric browser version (`0.2.1`); the `b3` suffix is a release-asset build identifier. Use `node scripts/build-release.mjs <version> <bN>` to produce a later build.
