...
헤드리스 브라우저 없음. 재로그인 없음. 비침습적. 실제 Chrome, 쿠키, 세션을 그대로 사용.

## 주요 특징

| 특징 | 설명 |
|---|---|
| 원프롬프트 설정 | 프롬프트 하나만 복사 & 붙여넣기. 설정 파일 불필요 |
| 근육 기억 | AI가 한 번 탐색 → 저장 → 다음에는 제로 토큰으로 재생. 85.7% 토큰 절약 · 80-95% 시간 단축 |
| 제로 침입 | 기존 Chrome에서 작동. 쿠키, 로그인, 확장 프로그램 유지 |
| 100% 로컬 | Bridge는 localhost에서 실행. 분석·추적·클라우드 없음 |
| 도구 세트 | 16개의 표준화된 도구 — Unix 철학으로 최소 인터페이스, 최대 조합 가능성 |

## 빠른 시작

1. **설치** — [zip 다운로드](https://github.com/hooosberg/AgentLimb/releases/latest) 후 압축 해제, `chrome://extensions` 열기, **개발자 모드** 활성화, **압축해제된 항목 로드** 클릭 후 폴더 선택
2. **복사** — 사이드 패널에서 "온보드 프롬프트 복사" 클릭
3. **붙여넣기** — AI 도구에 붙여넣기 → 자동 구성 후 즉시 작동

## 리소스

- [웹사이트](https://agentlimb.com) · [튜토리얼](https://agentlimb.com/tutorials.html) · [AI 도구](https://agentlimb.com/tools.html)

## 라이선스

[BSL 1.1](../LICENSE) — 개인 사용 무료. 상업적 사용은 라이선스 필요. 2030-04-12에 Apache 2.0으로 전환.
