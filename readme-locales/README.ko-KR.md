...
### 6. 멀티 계정 병렬 제어 — 여러 Chrome 프로필 동시 작동

AI 명령 하나로 모든 Chrome 프로필이 동시에 실행됩니다. Twitter 계정이 2개든, 회사 Google 계정이 3개든, 테스트 프로필이 수십 개든——AgentLimb은 하나의 작업에서 모두 제어합니다.

- **명시적 신원** — 각 사이드 패널에 "이 패널: Profile-xxxxxx" 표시；Bridge는 어떤 프로필이 어떤 결과를 반환했는지 파악
- **일시 정지 / 자동 일시 정지** — 패널을 닫거나 일시 정지 버튼을 클릭하면 해당 프로필이 작업에서 빠지고, 나머지는 계속 실행
- **task\_\* 브로드캐스트** — `task_plan`, `task_step_done`, `task_complete`, `task_fail`이 모든 활성 프로필에 브로드캐스트；모든 사이드 패널 동기화
- **윈도우 잠금** — `navigate`는 동일 프로필에 여러 창이 열려 있어도 사이드 패널이 열린 Chrome 창을 자동으로 타겟
- **타겟 라우팅** — 특정 프로필만 조작할 때 도구 호출 시 레이블로 지정 가능
...
| **멀티 계정 병렬** | 수동 오케스트레이션 필요 | 플랫폼에 따라 다름 | 미지원 | ✅ 여러 Chrome 프로필, 명령 하나로 |
