...
### 6. التحكم المتوازي بحسابات متعددة — تشغيل ملفات تعريف Chrome متعددة في آنٍ واحد

أمر ذكاء اصطناعي واحد، وكل ملف تعريف Chrome ينفّذه في آنٍ واحد. سواء كانت حسابَي Twitter، أو ثلاثة حسابات Google للشركة، أو عشرات ملفات التعريف التجريبية——يتحكم AgentLimb في جميعها ضمن مهمة واحدة.

- **هوية صريحة** — يعرض كل لوح جانبي "هذا اللوح: Profile-xxxxxx"؛ يعرف Bridge أي ملف تعريف أعاد أي نتيجة
- **الإيقاف المؤقت / الإيقاف التلقائي** — أغلق اللوح (أو انقر على إيقاف مؤقت) لإخراج ملف التعريف هذا من المهمة؛ يستمر الباقون دون انقطاع
- **بث task\_\*** — تُبثّ `task_plan` و`task_step_done` و`task_complete` و`task_fail` إلى كل ملف تعريف نشط؛ تبقى جميع الألواح الجانبية متزامنة
- **قفل النافذة** — يستهدف `navigate` نافذة Chrome الصحيحة تلقائيًا، حتى لو كان نفس ملف التعريف قد فتح نوافذ متعددة
- **توجيه الهدف** — حدّد ملف التعريف بالتسمية في استدعاء الأداة للحصول على دقة جراحية
...
| **متوازي متعدد الحسابات** | تنسيق يدوي | يتفاوت حسب المنصة | لا | ✅ ملفات Chrome متعددة، أمر واحد |
