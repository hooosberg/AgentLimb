...
### 6. マルチアカウント並列制御 — 複数の Chrome プロファイルを同時に操作

AIコマンド1つで、すべてのChromeプロファイルが同時に実行。Twitterアカウントが2つ、会社のGoogleアカウントが3つ、テストプロファイルが10個以上あっても——AgentLimbは1つのタスクでそのすべてを制御できます。

- **明示的な身元** — 各サイドパネルに「このパネル: Profile-xxxxxx」が表示；Bridgeはどのプロファイルがどの結果を返したか把握する
- **停止 / 自動停止** — パネルを閉じる（または停止ボタンをクリック）と、そのプロファイルがタスクから離脱し、他は継続して動作する
- **task\_\* ブロードキャスト** — `task_plan`、`task_step_done`、`task_complete`、`task_fail` がすべてのアクティブなプロファイルにブロードキャスト；すべてのサイドパネルが同期
- **ウィンドウロック** — 同じプロファイルで複数のウィンドウが開いていても、`navigate` はサイドパネルが開いているChromeウィンドウに自動的にターゲットする
- **ターゲットルーティング** — 特定のプロファイルのみを操作する場合、ツール呼び出しでラベル指定が可能
...
| **マルチアカウント並列** | 手動オーケストレーションが必要 | プラットフォーム次第 | 非対応 | ✅ 複数Chromeプロファイル、1コマンドで |
