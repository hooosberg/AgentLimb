...
3. **粘贴给你的 AI 终端** — 任何能执行本机终端命令的 agent 都遵循同一套 Windows 或 macOS 协议：先征得你的明确安装授权，再从 GitHub 下载与扩展版本匹配的 AgentLimb tag 源码归档，在临时目录解压后完成 Bridge 与 Native Messaging 配置并验证健康检查。它不会扫描浏览器 Profile、目录或扩展源码。
